...
- [Thymeleaf](#thymeleaf)
- [REST APIs](#rest-apis)
- [Controller Advice и обработване на грешки](#controller-advice)
- [Jackson JSON](#jackson-json)
- [Swagger UI - тесване на REST APIs](#swagger-ui)
...
1. Потребителят изпраща HTTP заявка към приложението (напр. `/api/deliveries`).
...
>Примери:
>- @RequestMapping("/api/deliveries") задава основния път за всички методи в контролера.
>- @RequestMapping(method = RequestMethod.GET) обработва GET заявки към /api/deliveries.
...
В Spring MVC Model и ModelAndView са основни механизми за предаване на данни от
контролера към view слоя (например JSP, Thymeleaf или друг template engine).
Те се използват в класическата MVC архитектура, където имаш ясно разделение
между контролер, модел и изглед.
...
>Примерен URL: `/api/deliveries/17`  
...
<a id="controller-advice"></a>
## VII. Controller Advice и обработване на грешки

`@ControllerAdvice` в Spring е механизъм за глобално обработване на изключения и
обща логика за всички контролери в приложението. Той се използва най-често за
централизирано управление на грешки в Spring MVC и REST API, вместо да пишеш
try-catch във всеки контролер поотделно.

Когато в едно Spring приложение възникне изключение (exception) в някой контролер,
по подразбиране то ще се върне като грешка към клиента и може да доведе до
неясен или стандартен HTTP отговор. За да се направи обработката
по-структурирана и предвидима, се използва `@ControllerAdvice`.
Тази аннотация казва на Spring, че даденият клас ще съдържа глобална логика,
която се прилага към всички контролери в приложението.

Вътре в клас, маркиран с `@ControllerAdvice`, се използва най-често аннотацията
@ExceptionHandler. Тя позволява да дефинираш методи, които да „хващат“ конкретни
типове изключения. Например, ако имаш UserNotFoundException, можеш да създадеш
метод, който я обработва и връща подходящ HTTP отговор, като например 404 Not Found.
Вместо приложението да се „счупи“ или да върне stack trace, клиентът получава чист
и разбираем JSON отговор с информация за грешката.

![Controller Advice Example](images/1.controller-advice.png) 

Когато работиш с REST API, често `@ControllerAdvice` се комбинира с `@ResponseBody`
(или директно се използва `@RestControllerAdvice`, което е съкратена форма).
Така всички error responses се връщат като JSON, а не като HTML страници.
Това е много важно за frontend приложения или мобилни клиенти, които очакват
структурирани данни.

Предимството на този подход е, че централизираната обработка на грешки прави кода
по-чист и по-лесен за поддръжка. Вместо да повтаряш една и съща логика във всеки
контролер, имаш едно място, където дефинираш как се обработват различните типове
грешки. Освен това позволява да стандартизираш отговорите – например всички
грешки да имат една и съща структура с поле за съобщение, статус код и timestamp.

<a id="jackson-json"></a>
## VIII. Jackson JSON

Jackson JSON е библиотека за Java, която се използва за преобразуване между Java обекти и JSON формат.
В контекста на Spring MVC и Spring REST APIs тя има много важна роля, защото позволява автоматична
комуникация между backend-а (който работи с Java обекти) и frontend или външни клиенти
(които обикновено използват JSON).

Когато създадеш REST API със Spring Boot и използваш аннотацията `@RestController`, 
Spring автоматично използва Jackson, за да конвертира Java обектите, които връщаш от контролера,
в JSON формат. Например, ако имаш метод, който връща обект User, Spring няма да върне директно
Java обект, а ще го превърне в JSON чрез Jackson и ще го изпрати като HTTP response.
Това се нарича serialization – процесът Java обект → JSON.

Обратният процес също се поддържа. Когато клиент изпрати JSON към REST API
(например при POST или PUT заявка), Spring използва Jackson, за да преобразува този JSON
в Java обект. Това става чрез аннотацията `@RequestBody`. Този процес се нарича
deserialization – JSON → Java обект. Така разработчикът не трябва ръчно да парсва JSON, 
всичко става автоматично.

![Jackson-JSON-Example](images/2.jackson-json.png)

Примерно body на HTTP Response от горният пример `Delivery` е сериализиран в `JSON` формат
```json
{
  "courier": {
    "firstName": "Ivan",
  ...
  "createdAt": "2026-05-17T11:38:52.314039",
  "customer": {
    "id": 1,
    "firstName": "Ivan",
    "lastName": "Ivanov",
  ...
  "estimatedArrivalAt": "2026-05-18T11:38:52.314039",
  "id": 2,
  "warehouse": {
    "id": 1,
    "city": "Plovdiv"
...
Jackson работи в Spring чрез механизъм, наречен `HttpMessageConverter`. Това е компонент, който
Spring използва, за да преобразува HTTP заявки и отговори в подходящи Java обекти и обратно.
Jackson е default конверторът за JSON в Spring Boot, когато използваш зависимостта
spring-boot-starter-web, което означава, че не е нужно да го конфигурираш ръчно в повечето случаи.

![Jackson-JSON-Annotations](images/3.jackson-json-annotations.png)

Освен основната си функция за преобразуване, Jackson предоставя и различни аннотации,
с които можеш да контролираш как точно изглежда JSON. Например `@JsonProperty` позволява
да промениш името на поле в JSON, `@JsonIgnore` позволява да скриеш определени полета
(например пароли), а `@JsonFormat` се използва за форматиране на дати.
Това е много полезно, когато искаш да разделиш вътрешната структура на Java обекта
от публичния JSON, който се изпраща към клиента.

<a id="swagger-ui"></a>
## IX. Swagger UI - тесване на REST APIs

Swagger UI е интерактивен инструмент за визуализация на RESTful API-та. Той предоставя уеб-базирано
потребителско интерфейсно представяне на API документация, позволявайки на разработчиците и тестерите
да разглеждат, тестват и взаимодействат с API крайни точки (endpoints) директно през браузъра.

Интеграция на Swagger UI със Spring Boot
```xml
<dependency>
...
Достъп до Swagger UI чрез браузъра на URL:
http://localhost:8090/swagger-ui/index.html

Swagger автоматично ще генерира документацията от анотациите върху твоите контролери


![Swagger UI](images/4.swagger-ui.png)

<a id="resources"></a>
## X. Допълнителни материали
